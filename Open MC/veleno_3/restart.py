...
def main():
    ...
    for moltiplicatore in moltiplicatori_to_test:
        ...
        # 1. Definiamo i percorsi: Vecchia cartella e Nuova cartella di RESTART
        old_work_dir = os.path.join(root_dir, f"ALTA_STAT_depletion_pitch_{moltiplicatore:.3f}".replace('.', '_'))
        new_work_dir = old_work_dir + "_RESTART"
        os.makedirs(new_work_dir, exist_ok=True)
        
        old_results_file = os.path.join(old_work_dir, "depletion_results.h5")
        if not os.path.exists(old_results_file):
            print(f"ATTENZIONE: File {old_results_file} mancante. Impossibile fare il restart.")
            continue
        
        print(f"\n{'='*50}\nAVVIO RESTART DEPLETION: Moltiplicatore {moltiplicatore:.3f}\nWorkspace: {new_work_dir}\n{'='*50}")
        
        os.chdir(new_work_dir)
        
        try:
            # 2. Sincronizziamo gli ID (fondamentale per evitare il plot piatto)
            openmc.reset_auto_ids()
            
            # 3. Generiamo la geometria di base
            model = build_model(moltiplicatore, p, a_int, a_ext, t_fuel, t_water, perc_water, root_dir)
            model.export_to_xml()
            
            # 4. Estraiamo i materiali vecchi dallo STEP 13 (Fine dei 400 giorni)
            print("Caricamento composizione isotopica allo step 13...")
            vecchi_risultati = openmc.deplete.Results(old_results_file)
            mats_restart = vecchi_risultati.export_to_materials(13)
            
            # 5. Ripristiniamo S(alpha, beta) per l'acqua (viene perso nell'H5)
            for mat in mats_restart:
                if mat.name == 'water':
                    mat.add_s_alpha_beta('c_D_in_D2O')
                    mat.add_s_alpha_beta('c_H_in_H2O')
            mats_restart.cross_sections = f"{base_path}/cross_sections.xml"
            
            # 6. Sostituiamo i materiali freschi del modello con quelli estratti
            model.materials = mats_restart
            
            # 7. Nuovo operatore
            operator = openmc.deplete.CoupledOperator(model, chain_file, normalization_mode="source-rate")
            
           # 8. Definiamo i NUOVI time_steps (Solo il pezzettino nuovo)
            time_steps = [0.001]
            
            # 9. Definiamo i source_rates: S_rate per il pezzettino acceso
            source_rates = [S_rate]
            ...
            print(f"-> Restart concluso con successo per moltiplicatore {moltiplicatore:.3f}.")
