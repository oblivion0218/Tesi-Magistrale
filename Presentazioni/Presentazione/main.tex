...
\begin{frame}{Evoluzione Temporale di $^{135}\text{I}$ e $^{135}\text{Xe}$}
    \begin{center}
        \includegraphics[height=0.65\textheight]{Xe.png} % Sostituisci con il nome del tuo file
    \end{center}
    \vfill
    \begin{flushleft}
        \scriptsize
        \textbf{Figura X:} Andamento accoppiato delle densità atomiche dello Iodio-135 e dello Xenon-135 durante l'operatività nominale e nella fase successiva allo spegnimento del reattore (\textit{shutdown}). 
\end{frame}

% --- SLIDE 2: FOTO SULLO XENON ---
\begin{frame}{Evoluzione Temporale di $^{135}\text{I}$ e $^{135}\text{Xe}$}
    \begin{center}
        \includegraphics[height=0.65\textheight]{Xe_zoom.png} % Sostituisci con il nome del tuo file
    \end{center}
    \vfill
    \begin{flushleft}
        \scriptsize
        \textbf{Figura X:} Il grafico mostra meglio l'insorgenza del picco di avvelenamento post-climax indotto dal disaccoppiamento dei tempi di dimezzamento dei due nuclidi.
    \end{flushleft}
\end{frame}
...
\begin{frame}{Analisi della Cinetica dello Xenon-135}
    \begin{itemize}
        \item \textbf{Fase Operativa (Equilibrio):} Durante il funzionamento stazionario, la produzione di $^{135}\text{Xe}$ (da fissione diretta e decadimento del $^{135}\text{I}$) è bilanciata perfettamente dalla sua distruzione per decadimento naturale e, soprattutto, per cattura neutronica parassita ($n,\gamma$).
        \item \textbf{Transitorio di Transizione (Xenon Pit):} Allo spegnimento del reattore, il flusso neutronico crolla istantaneamente ($\phi \to 0$), annullando il termine di rimozione per cattura ($\sigma_{a,\text{Xe}}\phi N_{\text{Xe}} \to 0$).
        \item \textbf{Meccanismo del Picco:} Il $^{135}\text{I}$ accumulato nel core continua a decadere in $^{135}\text{Xe}$ con un tempo di dimezzamento inferiore ($T_{1/2} = 6.6$ h) rispetto a quello di distruzione dello Xenon ($T_{1/2} = 9.2$ h). Questo dislivello cinetico genera un accumulo netto di veleno che raggiunge un massimo assoluto prima di decadere asintoticamente.
    \end{itemize}
\end{frame}
...
\begin{frame}{Evoluzione Temporale di $^{149}\text{Pm}$ e $^{149}\text{Sm}$}
    ...
    \begin{flushleft}
        \scriptsize
        \textbf{Figura Y:} Concentrazione del Promezio-149 e del Samario-149 in funzione del tempo di irraggiamento e dopo lo spegnimento. Si evidenzia il comportamento asintotico non decrescente del $^{149}\text{Sm}$ nella fase post-operativa dovuto alla stabilità intrinseca dell'isotopo.
    \end{flushleft}
\end{frame}
...
\begin{frame}{Analisi della Cinetica del Samario-149}
    \begin{itemize}
        \item \textbf{Indipendenza dal Flusso all'Equilibrio:} A differenza dello Xenon, la densità stazionaria di $^{149}\text{Sm}$ non dipende quantitativamente dal flusso neutronico totale $\phi$, poiché il flusso si semplifica analiticamente nel rapporto tra il tasso di produzione e quello di bruciamento (\textit{burn-out}).
...
\begin{frame}{Evoluzione della Potenza Termica del Core}
    \begin{center}
        \includegraphics[height=0.65\textheight]{power.png} % Sostituisci con il nome del tuo file
    \end{center}
    \vfill
    \begin{flushleft}
        \scriptsize
        \textbf{Figura Z:} Profilo temporale della potenza termica globale espressa in MW nel corso del ciclo di vita del combustibile. La stabilità del rateo riflette l'equilibrio tra la sorgente esterna fotonica costante e le proprietà moltiplicative macroscopiche della matrice sottocrtica.
    \end{flushleft}
\end{frame}
...
\begin{frame}{Evoluzione del Fattore di Moltiplicazione Effettivo ($k_{eff}$)}
    \begin{center}
        \includegraphics[height=0.65\textheight]{keff.png} % Sostituisci con il nome del tuo file
    \end{center}
    \vfill
    \begin{flushleft}
        \scriptsize
        \textbf{Figura K:} Andamento del fattore di moltiplicazione effettivo $k_{eff}$ in funzione dei giorni di operatività in piena potenza (HFP). Il tracciato documenta i margini di sicurezza legali e la stabilità complessiva del sistema rispetto all'avvelenamento e al consumo di fissile.
    \end{flushleft}
\end{frame}

\begin{frame}{Evoluzione del Fattore di Moltiplicazione Effettivo ($k_{eff}$)}
    \begin{center}
        \includegraphics[height=0.65\textheight]{k_eff_zoom.png} % Sostituisci con il nome del tuo file
    \end{center}
    \vfill
    \begin{flushleft}
        \scriptsize
...
\begin{frame}{Correlazione tra Potenza e Reattività nel Ciclo di Vita}
    \begin{itemize}
        \item \textbf{Stabilità della Potenza Termica:} In un ADS operante in regime stazionario, la potenza termica totale è linearmente vincolata all'intensità della sorgente esterna $S$. Il mantenimento di un profilo stabile della potenza dimostra l'efficace accoppiamento energetico tra la sorgente fotonucleare centrale e il core moltiplicante.
        \item \textbf{Deriva Cinetica del $k_{eff}$:} L'andamento temporale del $k_{eff}$ mostra l'impatto combinato dell'accumulo dei prodotti di fissione parassiti ($^{135}\text{Xe}$ e $^{149}\text{Sm}$) e del progressivo consumo dell'isotopo fissile. 
        \item \textbf{Effetto di Compensazione del Breeding:} Nello spettro epitemico/veloce considerato, il consumo di fissile è parzialmente compensato dal meccanismo di \textit{breeding} endogeno da nucleo fertile. Questo bilanciamento isotopico stabilizza l'autovalore nel lungo periodo, minimizzando le oscillazioni di reattività e garantendo la persistenza dei margini di sottocriticità impostati a freddo ($k_{max} < 1$).
    \end{itemize}
\end{frame}
