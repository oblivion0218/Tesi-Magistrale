...
\begin{frame}{Evoluzione Temporale di $^{135}\text{I}$ e $^{135}\text{Xe}$}
    ...
    \begin{flushleft}
        \scriptsize
        \textbf{Figura:} Il grafico mostra meglio l'insorgenza del picco di avvelenamento post-climax indotto dal disaccoppiamento dei tempi di dimezzamento dei due nuclidi.
    \end{flushleft}
\end{frame}
...
\begin{frame}{Analisi della Cinetica dello Xenon-135}
    \begin{itemize}
        \item \textbf{Produzione:} Il $^{135}\text{Xe}$ è prodotto sia direttamente dalla fissione sia indirettamente tramite il decadimento del $^{135}\text{I}$.
        \item \textbf{Fase Operativa (Equilibrio):} Durante il primo funzionamento, la produzione di $^{135}\text{I}$ è molto elevata, la sua eccessiva presenza porta a un crollo della reattività. La stabilità si raggiunge quando il tasso di produzione e quello di rimozione (decadimento + cattura) si bilanciano, determinando una densità stazionaria di $^{135}\text{Xe}$ che dipende dal flusso neutronico.
...
\begin{frame}{Evoluzione Temporale di $^{149}\text{Pm}$ e $^{149}\text{Sm}$}
    \begin{center}
        \includegraphics[height=0.75\textheight]{sm.png} % Sostituisci con il nome del tuo file
    \end{center}
    \vfill
    \begin{flushleft}
        \scriptsize
