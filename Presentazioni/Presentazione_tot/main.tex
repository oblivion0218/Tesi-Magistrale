...
\begin{frame}{Valutazione del $k_{max}$ e Analisi Parametrica}
    ...
    \begin{equation*}
        k_{max} = 1 - \Delta\rho_{doppler} - \Delta\rho_{water} - \Delta\rho_{poison} -\Delta\rho_{ignoranza} - \dots
    \end{equation*}
...
\begin{frame}{Analisi Accoppiata: Potenza e Moltiplicazione vs Pitch}
    \begin{figure}
        ...
        \includegraphics[width=0.6\linewidth]{potenza_kmax_vs_pitch.png}
...
\begin{frame}{Correlazione tra Potenza e Reattività nel Ciclo di Vita}
    \begin{itemize}
        \item \textbf{Calo di Potenza e Mitigazione:} Il decadimento della potenza termica nel tempo rappresenta una criticità operativa significativa. Tecniche di appiattimento del flusso (es. avvelenatori bruciabili come il Gadolinio) possono mitigare questo effetto, ma esulano dallo scopo del presente lavoro.
        
        \item \textbf{Dinamica del $k_{eff}$ e Veleni:} Il fattore di moltiplicazione evolve nello spazio delle fasi principalmente a causa della cinetica dei veleni neutronici. Ai fini di questa analisi, ci si concentra sulla quantificazione della differenza di reattività conseguente allo spegnimento del reattore.
        
        \item \textbf{Ipotesi di Lavoro:} Si assume $k_{eff}$ costante durante l'intero ciclo operativo. Il confronto avviene tra tre stati: il valore allo spegnimento ($k_{\text{off}}$), la massima reattività asintotica in HFP ($k_{\text{max}}$), e il riferimento ideale senza veleni ($k_{\text{id}}$).
    \end{itemize}
\end{frame}
...
\begin{frame}{Differenza di reattività post-spegnimento}
        ...
        \begin{tabular}{lcc}
            \toprule
            \textbf{Stato Operativo} & \textbf{Tempo $t$ [giorni]} & \textbf{Autovalore $k$} \\
            \midrule
            Spegnimento ($k_{\text{off}}$) & 400 & $0.95910 \pm 0.00035$ \\
            Massima reattività asintotica ($k_{\text{max}}$) & 402 & $0.96272 \pm 0.00036$ \\
            Riferimento ideale ($k_{\text{id}}$) & — & $0.97547 \pm 0.00020$ \\
            \bottomrule
        \end{tabular}
        \vspace{0.3em}
    ...
    \begin{alertblock}{Reattività da avvelenamento (riscalata)}
        \begin{equation*}
            \Delta\rho_{\text{Poison}} = \frac{k_{\text{max}} - k_{\text{off}}}{k_{\text{id}} \cdot (k_{\text{id}} + \Delta k)} = \boxed{424.8 \pm 25.4 \text{ pcm}}
        \end{equation*}
    \end{alertblock}

    \vfill
    \centering
    \footnotesize \textit{Il contributo dei veleni rappresenta circa il 13\% del margine totale di reattività.}

\end{frame}


    
\begin{frame}{Differenza di reattività totale per pitch = 3.551 cm}

    \begin{block}{Bilancio di reattività richiesto}
        \begin{equation*}
            \Delta\rho_{\text{tot}} = |\Delta\rho_{\text{Doppler}}| + |\Delta\rho_{\text{Moderator}}| + |\Delta\rho_{\text{Poison}}| + |\Delta\rho_{\text{Security}}|
        \end{equation*}
    \end{block}
    ...
    \begin{table}[h]
        ...
        \begin{tabular}{lc}
            \toprule
            \textbf{Contributo} & \textbf{Valore [pcm]} \\
            \midrule
            Doppler & 417 \\
            Moderatore & 1897 \\
            Veleni (Sm + Xe) & 425 \\
            Margine di sicurezza & 200 \\
            \midrule
            \textbf{Totale} & \textbf{2939} \\
            \bottomrule
        \end{tabular}
    \end{table}
    ...
    \begin{alertblock}{Limite operativo}
        \begin{equation*}
            k^{\text{HFP}}_{\text{max}} = 1 - \Delta\rho_{\text{tot}} = \mathbf{0.97061}
        \end{equation*}
    \end{alertblock}

    \vfill
    \centering
    \small Restando sotto questo limite, il sistema opera sempre in condizioni di sicurezza.
...
\begin{frame}
    ... e con questo concludo la presentazione, grazie per l'attenzione!
\end{frame}
